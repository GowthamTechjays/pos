...
export const saveSalesHubAction =
  ...
  (dispatch: AppDispatch) => {
    ...
    formData.append(
      ...
      JSON.stringify(payload.solutionNarrativeId)
    );
    ...
    if (salesHubId) {
      ...
      multiPartRequest(`partnership/sales-hub/`, formData, {
        ...
        if (resp.result === true) {
          loaderAction();
          history.push(
            `/salesHub?partner_ship_id=${partnershipId}&sales_hub_id=${resp.data.sales_hub_id}`
          );
          showAlert(resp.msg);
          cbFunc();
        } else {
...
export const deleteSolutionNarrativeAction =
  (
    partnershipId: string,
    salesHubId: string,
    loaderAction: () => void,
